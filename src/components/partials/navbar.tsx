  ...
  return (
    <>
      <header className="fixed top-0 w-full z-10">
        <div className="relative flex items-center justify-between px-6 md:px-24 py-4 bg-white/50s backdrop-blur border-b border-neutral-100">
          ...
          <div className="hidden md:block">
            <Link href="/contact" className={`${pathname == '/contact' ? 'bg-linear-to-tl from-red-500 to-red-300 text-white border-red-200' : 'border-neutral-200 hover:bg-linear-to-tl hover:from-red-500 hover:to-red-300 hover:text-white hover:border-red-200'} border-2 font-medium rounded-full px-8 py-2 text-sm transition cursor-pointer`}>
              Order Now
            </Link>
          </div>

          <div className="md:hidden flex">
            <button onClick={() => toggleMenu()} className="border border-neutral-200 p-2 rounded-lg">
              {isMenuOpen ? (
                <X />
              ) : (
                <Menu />
